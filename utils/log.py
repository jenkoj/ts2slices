...
def print_parameters(par: dict) -> None:
    ...
    print_log(par,
        ...
        " save signal: "f"{par['sig_save']}","\n"
        " save state: "f"{par['state_save']}","\n"
...
def print_progress(i: int, signal_stack: np.ndarray, sig_stack: np.ndarray, next_percent:int, par: dict) -> int:
    """
    Prints current progress of transformation every 10 %.
    """
    signal_slices_len = signal_stack.shape[0]
    sig_stack_len = sig_stack.shape[0]
    
    if round(100*i/(signal_slices_len),2) > next_percent:
        next_percent += 10
        print_log(par,"processed: "f"{round(100*i/(signal_slices_len), 2)} % finished: "f"{round(100*((sig_stack_len)/par['max_images']), 2)} %")
...
def print_end_of_loop(images_stacked: int, appliance: str, par: dict):
    ...
    """
    print_log(par,"\n")
    print_log(par,"number of slices (per appliance) stacked: "f"{images_stacked}")
...
def print_end(all_images_stacked: int, healthy_appliances: int, par: dict):
    ...
    """
    print_log(par,"num of slices stored: ", all_images_stacked)
